...
const calculatorTag = '.j-calculator',
    sCssLoading = 'loading';
...
window.jCalculator = (function (that) {
    ...
    that.changeMode = function (value, el) {
        $(el).attr('data-mode', value);
    };

    that.setReservedCloudlets = function (cloudlets, el, type) {
        $(el).attr('data-' + type + '-reserved', cloudlets);
        that.setMinValues(el, type);
    };

    that.setScalingCloudlets = function (cloudlets, el, type) {
        if (cloudlets === 0) {
        ...
        that.setMaxValues(el, type);
    };

    that.getReservedCloudlets = function (el, type) {
        return parseInt($(el).attr('data-' + type + '-reserved'));
    };

    that.getScalingCloudlets = function (el, type) {
        return parseInt($(el).attr('data-' + type + '-scaling'));
    };

    that.convertMib = function (value) {
        value *= 128;
        return value > 1000 ? parseFloat(value / 1024).toFixed(2) + " GiB" : value + " MiB";
    };

    that.convertMhz = function (value) {
        value *= 400;
        return value > 1000 ? parseFloat(value / 1000).toFixed(2) + " GHz" : value + " MHz";
    };

    that.changePricePeriod = function (sValue, sPeriod) {
        switch (sPeriod) {
            case 'hourly':
    ...
    that.checkStoragePrice = function (sValue, tiers) {
    ...
    that.checkIpPrice = function (sValue, tiers) {
        sValue = parseInt(sValue);
        var price = tiers[0].price;
    ...
    that.checkTrafficPrice = function (sValue, tiers) {
    ...
    that.setPrice = function (reservedTiers, scalingTiers, el, storageTiers, ipTiers, trafficTiers) {
        ...
        var balancerNodes = $(el).attr('data-balancer-nodes'),
            ...
            minBalancerPrice = that.checkPrice(parseInt(that.getReservedCloudlets(el, 'balancer')) * parseInt(balancerNodes), reservedTiers),
            minAppserverPrice = that.checkPrice(parseInt(that.getReservedCloudlets(el, 'appserver')) * parseInt(appServerNodes), reservedTiers),
            minDatabasePrice = that.checkPrice(parseInt(that.getReservedCloudlets(el, 'database')) * parseInt(databaseNodes), reservedTiers),
            maxBalancerPrice = that.checkMaxPrice(parseInt(that.getScalingCloudlets(el, 'balancer')) * parseInt(balancerNodes), scalingTiers, parseInt(that.getReservedCloudlets(el, 'balancer')) * parseInt(balancerNodes), reservedTiers),
            maxAppserverPrice = that.checkMaxPrice(parseInt(that.getScalingCloudlets(el, 'appserver')) * parseInt(appServerNodes), scalingTiers, parseInt(that.getReservedCloudlets(el, 'appserver')) * parseInt(appServerNodes), reservedTiers),
            maxDatabasePrice = that.checkMaxPrice(parseInt(that.getScalingCloudlets(el, 'database')) * parseInt(databaseNodes), scalingTiers, parseInt(that.getReservedCloudlets(el, 'database')) * parseInt(databaseNodes), reservedTiers),
            storagePrice = that.checkStoragePrice($(el).attr('data-storage'), storageTiers),
            ipPrice = that.checkIpPrice($(el).attr('data-ip'), ipTiers),
            trafficPrice = that.checkTrafficPrice($(el).attr('data-traffic'), trafficTiers);

        // TRAFFIC
        trafficPrice = that.toCurrency(trafficPrice, originalCurrency.rate.USD, currentCurrency.rate.USD);
        ...
        minPrice = that.toCurrency(minPrice, originalCurrency.rate.USD, currentCurrency.rate.USD);
        minPrice = that.changePricePeriod(minPrice, $(el).attr('data-period'));
        ...
        maxPrice = that.toCurrency(maxPrice, originalCurrency.rate.USD, currentCurrency.rate.USD);
        maxPrice = that.changePricePeriod(maxPrice, $(el).attr('data-period'));
        ...
        // RESERVED COUNTS
        var reservedBalancerCloudlets = parseInt(that.getReservedCloudlets(el, 'balancer')) * parseInt(balancerNodes),
            reservedAppServerCloudlets = parseInt(that.getReservedCloudlets(el, 'appserver')) * parseInt(appServerNodes),
            reservedDbCloudlets = parseInt(that.getReservedCloudlets(el, 'database')) * parseInt(databaseNodes),
            reservedCloudletsMib = that.convertMib(+reservedBalancerCloudlets + +reservedAppServerCloudlets + +reservedDbCloudlets),
            reservedCloudletsGHz = that.convertMhz(+reservedBalancerCloudlets + +reservedAppServerCloudlets + +reservedDbCloudlets);
        ...
        // SCALING COUNTS
        var scalingBalancerCloudlets = parseInt(that.getScalingCloudlets(el, 'balancer')) * parseInt(balancerNodes),
            scalingAppServerCloudlets = parseInt(that.getScalingCloudlets(el, 'appserver')) * parseInt(appServerNodes),
            scalingDbCloudlets = parseInt(that.getScalingCloudlets(el, 'database')) * parseInt(databaseNodes),
            scalingCloudletsMib = that.convertMib(+scalingBalancerCloudlets + +scalingAppServerCloudlets + +scalingDbCloudlets),
            scalingCloudletsGHz = that.convertMhz(+scalingBalancerCloudlets + +scalingAppServerCloudlets + +scalingDbCloudlets);
    ...
    that.checkPrice = function (cloudlets, tiers) {
    ...
    that.checkMaxPrice = function (cloudlets, tiers, minCloudlets, minTiers) {

        if (cloudlets === 0) {
            return 0;
        }

        var reservedPrice = that.checkPrice(minCloudlets, minTiers),
    ...
    that.toCurrency = function (nValue, sFrom, sTo) {
        return (sFrom / sTo) * nValue;
    };

    that.setMinValues = function (el, type) {


        var value = that.getReservedCloudlets(el, type);
        ...
        var mib = that.convertMib(value);
        var mhz = that.convertMhz(value);
        $(el).find('.' + type + '-range .min-block .digits').html('<span>' + mib + '</span><span>' + mhz + '</span>')

    };

    that.setMaxValues = function (el, type) {

        var value = that.getScalingCloudlets(el, type);
    ...
    that.decreaseReserved = function (oSlider) {
    ...
    that.increaseReserved = function (oSlider) {
        var l = oSlider.value.split(',')[0];
        var r = oSlider.value.split(',')[1];
    ...
    that.decreaseScaling = function (oSlider) {
        var l = oSlider.value.split(',')[0];
        var r = oSlider.value.split(',')[1];
    ...
    that.increaseScaling = function (oSlider) {
        var l = oSlider.value.split(',')[0];
        var r = oSlider.value.split(',')[1];
    ...
    that.increaseBlockDigit = function (clickedElement) {
        var digit = $(clickedElement).closest('.inner').find('.digit');
        digit.val(parseInt(digit.val()) + 1).change();
    };

    that.decreaseBlockDigit = function (clickedElement) {
        var digit = $(clickedElement).closest('.inner').find('.digit');
        if (parseInt(digit.val()) > 0) {
            digit.val(parseInt(digit.val()) - 1).change();
        }
    ...
    that.renderCalculator = function (el) {
        ...
        $(el).find('input[name*="mode"]').click(function (e) {
            that.changeMode($(this).val(), el);
            that.setMinValues(el, $(el).attr('data-mode'));
            that.setMaxValues(el, $(el).attr('data-mode'));
        });
        ...
        $(el).find('.digit, .node-count input').change(function (e) {
            ...
            that.setPrice(fixed.tiers, dynamic.tiers, el, storage.tiers, ip.tiers, network.tiers);
        });
        $(el).find('.plus').click(function (e) {
            e.preventDefault();
            that.increaseBlockDigit(this);
        });
        $(el).find('.minus').click(function (e) {
            e.preventDefault();
            that.decreaseBlockDigit(this);
        });
        $(el).find('.plus-node').click(function (e) {
            e.preventDefault();
            that.increseNode(this);
        });
        $(el).find('.minus-node').click(function (e) {
            e.preventDefault();
            that.decreaseNode(this);
        });
    ...
    that.fnSetDefault = function () {
        var sHosterCriteria = JApp.utils.uniqid();
        if (JApp.isLoadedDefHoster()) {
            return that.fnInitDefaultHoster();
        } else {
            return JApp.loadDefaultHoster(that.fnInitDefaultHoster, sHosterCriteria);
        }
    };

    that.fnInitDefaultHoster = function (sHoster) {
        sCurrentHoster = sHoster || JApp.getDefaultHoster();
        that.getAllPrices(function () {
            JApp.loadHosters(function (hosters) {
                ...
                $.each(calculatorsWithSelector, function () {
                    that.renderHosterSelector($(this));
                    that.renderCalculator(this);
                })
            });
        });
    };

    that.getAllPrices = function (fnCallback) {
        $.ajax({
            type: "GET",
    ...
    that.getHosterPrices = function (sKey, element) {
        $.ajax({
            type: "GET",
...
jQuery(document).ready(function ($) {
    ...
    var calculators = $(calculatorTag);
    calculators.addClass(sCssLoading);
        ...
        $.ajax({
            ...
            success: function (currencyJSON) {
                if (currencyJSON.result === 0) {
                    ...
                    $.each(calculators, function (e) {
                        if ($(this).attr('data-key')) {
                            jCalculator.getHosterPrices($(this).attr('data-key'), this);
                        } else {
...
$(window).resize(function () {
    $('.j-calculator[data-mode]').each(function () {
        jCalculator.setMinValues(this, $(this).attr('data-mode'));
        jCalculator.setMaxValues(this, $(this).attr('data-mode'));
    });
});