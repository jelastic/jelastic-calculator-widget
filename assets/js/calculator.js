...
JApp.pricing.Calculator = function (config) {
    ...
    self.renderCalculator = function () {
        ...
        var sHtml = new EJS({url: self.baseUrl + 'j-calculator/templates/j-calculator'}).render({
            localization: self.localization,
            ...
            $.ajax({
                ...
                success: function (hosterPricingJSON) {
                    if (hosterPricingJSON.result === 0 && hosterPricingJSON.response.models[0]) {
                        $.each(hosterPricingJSON.response.models, function (index, model) {
                            if (model.isDefault) {
                                self.pricing[self.sKey] = model;
                            }
                        });
